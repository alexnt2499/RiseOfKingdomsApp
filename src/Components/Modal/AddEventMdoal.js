    ...
    const addToDB = async () => {
        if(note) {
            ...
            try {
                ...
                let ids = `${id}`.substring(`${id}`.length - 5,`${id}`.length)
    ...
    return (
        ...
        isVisible={isModalVisible}>
          <View style={styles.container}>
            ...
            <Text style={[styles.textStyle,{marginTop : 25}]}>{t('add_event').toUpperCase()}</Text>
            ...
            <View style={{width : '100%', justifyContent : 'center', alignItems : 'flex-start',marginLeft : 50, marginTop : 10}}>
            <Text style={[styles.textStyle,{fontFamily : fonts.regular,fontSize : 14}]}>{t('chon_ngay').toUpperCase()}</Text>
            ...
            <Text style={[styles.textStyle,{color : '#FFF', fontFamily : fonts.light}]}>{t('tao_event')}</Text>
            </TouchableOpacity>
